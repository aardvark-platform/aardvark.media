...
module MutableApp =
    ...
    let private template = 
        ...
        use stream = assembly.GetManifestResourceStream(@"29_-_SubappGiraffeGoldenLayout.rtemplate.html")
    ...
    let toWebPart' (runtime : IRuntime) (useGpuCompression : bool) (app : MutableApp<'model, 'msg>) =
        ...
        let events (ws : WebSocket) (context: HttpContext) =
            match context.Request.Query.TryGetValue "session" with
                | (true, SingleString (Guid sessionId)) ->
                    //Log.line "[MutableApp] session id %s" (string sessionId)
                    let request = 
                        {
                    ...
                    let updateFunction () =
                        try
                                            ...
                                            o.EvaluateAlways AdaptiveToken.Top (fun t ->
                                                ...
                                                let code = 
                                                    ...
                                                    for (name, sd) in Dictionary.toSeq scenes do
                                                        Log.line "[MutableApp] scene name = %s" name
                            ...
                            if ws.State = WebSocketState.Open then // added because of Error when closing client
                    ...
                    let read buffer =
                        async {
                            try
                                match ws.State with
                                | WebSocketState.CloseReceived | WebSocketState.Closed ->
                                    ws.Dispose ()
                                    return Choice3Of3 (sprintf "[MutableApp] Client %s closed websocket." (string sessionId))
                                | _ ->
                                    let! r = ws.ReceiveAsync(ArraySegment(buffer), cts.Token)
                                    return Choice1Of3 r
                            with e ->
                                return Choice2Of3 e
                        }
                    ...
                    task {
                        try
                            while running && ws.State <> WebSocketState.Closed do
        ...
        let waitForShutdown = // TODO R4F no shutdown when client closes!
    ...
    let toWebPart (runtime : IRuntime) (app : MutableApp<'model, 'msg>) =
        toWebPart' runtime false app |> fst
