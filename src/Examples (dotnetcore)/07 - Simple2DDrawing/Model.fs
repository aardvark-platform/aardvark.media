...
[<ModelType>]
type Polygon = { points : list<V2d> } // polygon is a list of points, we use list<V2d> here, since tracking single points adaptively is kind of overkill here though IndexList<V2d> would work as well.
...
type Model =
    {
        finishedPolygons : IndexList<Polygon> // we use plist here, since plist is automatically mapped to alists in the adaptive version (See https://rawgit.com/vrvis/aardvark.media/base31/docs/DomainTypeGeneration.html)

        workingPolygon : Polygon // the (unfinished) polygon we are working on
        cursor         : Option<V2d>     // the cursor

        [<TreatAsValue>]        // since we don't want to have automatically maintained incremental versions of the past, we treat this field as value (the adaptive version becoms aval<Option<Model>> instead of MOption<MModel>)
