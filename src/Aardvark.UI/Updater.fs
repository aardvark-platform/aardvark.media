...
module Updaters =
    ...
    and MapUpdater<'inner, 'outer>(m : MapNode<'inner, 'outer>, inner : IUpdater<'inner>) =
        ...
        let mutable cache : Option<UpdateState<'inner> * FakeRx.Subject<'inner>> = None
                ...
                | Some (_,subject) ->
                    ...
                    seq {
                        for msg in messages do
                            yield m.Mapping msg
                            subject.OnNext msg // if already destroyed, subapp does not receive msg any more...
                    }
                | _ ->
                    Seq.empty
        ...
        let get (state : UpdateState<'outer>) =
            ...
            | Some c -> c
            | None ->
                let subject = new FakeRx.Subject<'inner>()
                let innerState =
                    let test = state.scenes |> ContraDict.map (fun _ (scene, msg, getState) -> (scene, (msg |> Option.map (SceneMessages.map m.Mapping)), getState))
                    {
                        scenes              = test
    ...
    and SubAppUpdater<'model, 'inner, 'outer>(n : SubAppNode<'model, 'inner, 'outer>, m : MutableApp<'model, 'inner>, inner : IUpdater<'inner>) =
        ...
        let mutable cache : Option<UpdateState<'inner> * FakeRx.Subject<'inner> * IDisposable> = None
