...
module Incremental =
    ...
    let renderControl (cam : aval<Camera>) (attributes : AttributeMap<'msg>) (sg : ISg<'msg>) =
        DomNode.RenderControl(attributes, cam, sg, RenderControlConfig.standard)

    let renderControl' (cam : aval<Camera>) (attributes : AttributeMap<'msg>) (config : RenderControlConfig) (sg : ISg<'msg>) =
        DomNode.RenderControl(attributes, cam, sg, config)

    let renderControlWithClientValues (cam : aval<Camera>) (attributes : AttributeMap<'msg>) (sg : RenderClientValues -> ISg<'msg>) =
        DomNode.RenderControl(attributes, cam, sg, RenderControlConfig.standard)

    let renderControlWithClientValues' (cam : aval<Camera>) (attributes : AttributeMap<'msg>) (config : RenderControlConfig)
                                       (sg : RenderClientValues -> ISg<'msg>) =
        DomNode.RenderControl(attributes, cam, sg, config)
    ...
    let inline pre a x = DomNode.Text("pre", null, a, x)
    ...
    let inline textarea a x = DomNode.Text("textarea", null, a, x)
...
module Static =
    ...
    let subApp (app : IApp<'model, 'inner>) : DomNode<'outer> =
        DomNode.SubApp
            { new ISubApp<'model, 'inner, 'outer> with
                member x.Start() = app.Start()
                member x.ToInner (_,_)= Seq.empty
                member x.ToOuter (_,_) = Seq.empty
            }

    let subApp' (mapOut : 'model -> 'inner -> 'outer seq) (mapIn : 'model -> 'outer -> 'inner seq) (app : IApp<'model, 'inner>) : DomNode<'outer> =
        DomNode.SubApp
            { new ISubApp<'model, 'inner, 'outer> with
                member x.Start() = app.Start()
    ...
    let renderControl (cam : aval<Camera>) (attributes : list<string * AttributeValue<'msg>>) (sg : ISg<'msg>) =
        DomNode.RenderControl(AttributeMap.ofList attributes, cam, sg, RenderControlConfig.standard)
    ...
    let inline ol x = elem "ol" x
    let inline p x = elem "p" x
    let inline pre a x = DomNode.Text("pre", null, AttributeMap.ofList a, AVal.constant x)
    let inline section x = elem "section" x
    let inline ul x = elem "ul" x
    ...
    let inline progress x = elem "progress" x
    let inline select x = elem "select" x
    let inline textarea a x = DomNode.Text("textarea", null, AttributeMap.ofList a, AVal.constant x)
...
module HigherOrderTags =
    ...
    let onShutdown (code : string) (node : DomNode<'msg>) =
        ...
        | ValueNone -> node.WithShutdown (ValueSome shutdown)
        | ValueSome o -> node.WithShutdown (ValueSome (fun id -> shutdown id + "; " + o id))
    
    let onBoot (code : string) (node : DomNode<'msg>) =
        let boot id = code.Replace("__ID__", id)
        match node.Boot with
        | ValueNone -> node.WithBoot (ValueSome boot)
        | ValueSome o -> node.WithBoot (ValueSome (fun id -> boot id + "; " + o id))
    ...
    let onBoot' (channels : list<string * Channel>) (code : string) (node : DomNode<'msg>) =
        ...
        match node.Boot with
        | ValueNone -> node.WithBoot(ValueSome boot).WithChannels n
        | ValueSome o -> node.WithBoot(ValueSome (fun id -> boot id + "; " + o id)).WithChannels n
...
module Generic =
    ...
    type ContentCreator =
        ...
        static member inline Content(l : aval<string>) = { new IDomContent<'msg> with member x.Create(t,a) = DomNode.Text(t, null, a, l) }
        static member inline Content(l : string) = { new IDomContent<'msg> with member x.Create(t,a) = DomNode.Text(t, null, a, AVal.constant l) }
    ...
    type Creator =
        ...
        static member inline Node(tag : string, attributes : AttributeMap<'msg>, content : aval<string>) =
            DomNode.Text(tag, null, attributes, content)
            
        static member inline Node(tag : string, attributes : AttributeMap<'msg>, content : string) =
            DomNode.Text(tag, null, attributes, AVal.constant content)
        ...
        static member inline Node(tag : string, attributes : list<string * AttributeValue<'msg>>, content : aval<string>) =
            DomNode.Text(tag, null, AttributeMap.ofList attributes, content)
            
        static member inline Node(tag : string, attributes : list<string * AttributeValue<'msg>>, content : string) =
            DomNode.Text(tag, null, AttributeMap.ofList attributes, AVal.constant content)
