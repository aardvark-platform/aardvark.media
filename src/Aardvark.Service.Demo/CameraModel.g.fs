...
module Mutable =
    ...
    type MCameraControllerState private(__initial : Aardvark.UI.CameraControllerState) =
        ...
        member x.Update(__model : Aardvark.UI.CameraControllerState) =
    ...
    module CameraControllerState =
        ...
        module Lens =
            ...
            let moveVec =
                { new Lens<Aardvark.UI.CameraControllerState, Aardvark.Base.V3i>() with
                    override x.Get(r) = r.moveVec
                    override x.Set(r,v) = { r with moveVec = v }
                    override x.Update(r,f) = { r with moveVec = f r.moveVec }
                }
            let orbitCenter =
                { new Lens<Aardvark.UI.CameraControllerState, Microsoft.FSharp.Core.Option<Aardvark.Base.V3d>>() with
            ...
            let lastTime =
                { new Lens<Aardvark.UI.CameraControllerState, Microsoft.FSharp.Core.Option<Microsoft.FSharp.Core.float>>() with
                    override x.Get(r) = r.lastTime
                    override x.Set(r,v) = { r with lastTime = v }
                    override x.Update(r,f) = { r with lastTime = f r.lastTime }
                }
            let stash =
                { new Lens<Aardvark.UI.CameraControllerState, Microsoft.FSharp.Core.Option<Aardvark.UI.CameraControllerState>>() with
