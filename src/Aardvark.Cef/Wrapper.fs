...
type RenderProcessHandler() =
    ...
    override x.OnContextCreated(browser : CefBrowser, frame : CefFrame, context : CefV8Context) =
        context.Use (fun () ->
            ...
            let (success, res, exn) = context.TryEval(javascriptUtilities)
                    ...
                    match cmd with
                        | IPC.Execute(token, js) ->
                            ...
                            if isNull frame then
                                ...
                                ctx.Use (fun () ->
                                    let (success, res, exn) = ctx.TryEval js
                            ...
                            if token.IsValid then
                                ...
                                if isNull frame then
                                    ...
                                    ctx.Use (fun () ->
                                        ...
                                        let (success, res, exn) = ctx.TryEval (sprintf "aardvark.getViewport('%s')" id)
...
and LoadHandler(parent : Client) =
    ...
    override x.OnLoadStart(browser : CefBrowser, frame : CefFrame) =
